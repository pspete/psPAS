	...
	InModuleScope $(Split-Path (Split-Path (Split-Path -Parent $PSCommandPath) -Parent) -Leaf ) {
		...
		Context 'Mandatory Parameters' {
			...
			It 'has no mandatory parameters' {
				...
				$MandatoryParams = (Get-Command Remove-PASFIDO2OwnDevice).Parameters.Values | Where-Object { $_.Attributes.Mandatory -eq $true }
		...
		Context 'Input' {
			...
			BeforeEach {
				...
				$response = $InputObj | Remove-PASFIDO2OwnDevice
			...
			It 'throws error if version requirement not met' {
				...
				{ $InputObj | Remove-PASFIDO2OwnDevice } | Should -Throw
				$psPASSession.ExternalVersion = '0.0'
			}

	...
	InModuleScope $(Split-Path (Split-Path (Split-Path -Parent $PSCommandPath) -Parent) -Leaf ) {
		...
		Context 'Input' {
			...
			It 'does not send an additional request when WhatIf is used' {
				...
				$InputObj | Add-PASPolicyACL -UserName 'root' -Command 'some command' -CommandGroup $false -PermissionType Deny -WhatIf
			...
			It 'sends request to expected endpoint' {
				...
				Assert-MockCalled Invoke-PASRestMethod -ParameterFilter {
					...
					$URI -eq "$($Script:psPASSession.BaseURI)/WebServices/PIMServices.svc/Policy/UNIXSSH/PrivilegedCommands/"

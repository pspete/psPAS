...
function Set-PASVRMServiceConfig {
    ...
    param(
        ...
        [ValidateScript( {
                $allowedKeys = @(
                    ...
                    'ReplicateInterval',
                    'ReplicateRetriesInterval',
                    'DefaultTimeout',
                    'DebugLevel',
                    'LockTimeout'
                )
                $invalidKeys = $_.Keys | Where-Object { $_ -notin $allowedKeys }
                if ($invalidKeys) {
                    throw "Invalid parameter name(s): $($invalidKeys -join ', '). Allowed parameters: $($allowedKeys -join ', ')"
                }
                $true
            })]
    ...
    process {
        ...
        $URI = "$BaseURI/API/VaultActions/SetServiceConfig"

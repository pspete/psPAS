...
function Get-PASVRMServiceConfigParameter {
    [CmdletBinding()]
    param(
        [parameter(
            Mandatory = $false,
    ...
    process {
        ...
        $URI = "$BaseURI/API/VaultActions/GetServiceConfig/$parameterName"

        #Get Parameters for request body
        $boundParameters = $PSBoundParameters | Get-PASParameter -ParametersToRemove BaseURI, parameterName

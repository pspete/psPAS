...
psPAS offers over 250 PowerShell commands for interacting with the CyberArk / Idira REST API, covering everything from accounts, safes and users to platforms, discovery, reporting and CPM operations. Originally launched on GitHub in 2017, the module is now also available via the PowerShell Gallery for easy installation and updates.

Designed for flexibility, psPAS supports both the latest CyberArk releases - Self-Hosted, Privilege Cloud and Identity Shared Services alike - and legacy API versions, making it a powerful tool for automating everything from day-to-day admin tasks to complex operational workflows.
...
- Commands that behave like PowerShell, not like the REST API:
  - Standard Verb-Noun naming, pipeline support, and structured objects instead of raw JSON, so commands work the way PowerShell users already expect.
...
Help and support should be sought by opening an Issue on the project's GitHub page (preferable), or emailing <a href="mailto:pspas@pspete.dev">pspas@pspete.dev</a> if opening a GitHub Issue is not an available option.

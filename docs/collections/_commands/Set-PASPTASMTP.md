...
### Example 2
```powershell
PS C:\> Set-PASPTASMTP -host smtp.cyberark.local -protocol NONE -port 25 -sender 'pta-alerts@cyberark.local' -recipients 'soc-team@cyberark.local' -AlertToEmailScoreThreshold 80
```
...
### Example 3
```powershell
PS C:\> Set-PASPTASMTP -host smtp.cyberark.local -protocol STARTTLS -port 587 -sender 'pta-alerts@cyberark.local' -recipients 'soc-team@cyberark.local','ciso@cyberark.local' -accountId 34_5 -CertificateFile C:\Certs\smtp.cer -AlertToEmailScoreThreshold 60
```
...
### Example 4
```powershell
PS C:\> Set-PASPTASMTP -host smtp.cyberark.local -protocol SSL -port 465 -sender 'pta-alerts@cyberark.local' -recipients 'soc-team@cyberark.local' -CertificateFile C:\Certs\smtp.cer -AlertToEmailScoreThreshold 75 -WhatIf
```
